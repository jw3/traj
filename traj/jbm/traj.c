...
int trajectory_calculate(lpTrajectory trajectory)
{
	...
	i = (trajectory->range_max - trajectory->range_min + 1) / trajectory->range_inc + 1;
	...
	if (options_getoption(o, TRAJ_OPT_ALTI))
		atmos_standardalt(trajectory->atmos);
	...
	if (options_getoption(o, TRAJ_OPT_AZIM))
		...
		azim = trajectory->azimuth;

	if (options_getoption(o, TRAJ_OPT_ELEV))
		elev = 0.0;
	else
		elev = trajectory->elevation;
	...
	while (((err > TRAJ_ERROR) && (itcnt < TRAJ_MAXITCNT)) || (itcnt == 0)) {
		...
		for (i = 0; i <= k; i++) {
			...
			if ((vm < TRAJ_ABSMINVX) || (r.y < TRAJ_ABSMINY))
				break;
			if ((i >= trajectory->range_min) && (i <= trajectory->range_max) && (i % trajectory->range_inc == 0)) {
