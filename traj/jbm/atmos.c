...
void atmos_atmos(lpAtmos atmos)
{
	...
	if (t > 0.0) {
		...
		hc = 1.0;
	atmos->density = ATMOS_DENSSTD * (ATMOS_TSTDABS / (t + ATMOS_T0)) * hc;
...
void atmos_standardalt(lpAtmos atmos)
{
	...
	atmos->pressure = ATMOS_PRESSSTD * pow(ATMOS_TSTDABS / atmos->temperature, ATMOS_PRESSEXPNT);
	atmos->temperature = atmos->temperature - ATMOS_T0; /* line above need absolute! */
