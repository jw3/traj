...
double mcdrag_getrtr(lpMcDrag mcdrag)
{
	...
	switch (mcdrag->nst)
	{
		...
		case secant:
			...
			x0 = mcdrag->rad * cos(b);
			y0 = mcdrag->rad * sin(b);
			tns = mcdrag->nsl + sqrt(SQR(mcdrag->rad) - SQR(y0 - 0.5*mcdrag->dmp)) - x0;
...
double mcdrag_cd_bttl(lpMcDrag mcdrag, double mach)
{
	...
	if ((mach <= 0.85) || (mcdrag->btl == 0.0))
		...
		b = sqrt(m2 - 1.0);
		b3 = 0.85 / b;
		a12 = (5.0 * tau) / (6.0 * b) + pow(0.5 * tau, 2.0) - (0.7435 / m2) * pow(tau * mach, 1.6);
...
double mcdrag_cd_base(lpMcDrag mcdrag, double mach)
{
	...
	pb4 = (1.0 + 0.09 * m2 * (1.0 - exp(mcdrag->nsl - mcdrag->tln))) * (1.0 + 0.25 * m2 * (1.0 - mcdrag->dba));
...
double mcdrag_twist(lpMcDrag mcdrag, double mach, double *stability)
{
	...
	if (mach < 1.0) {
		...
		a = 34.779 + (24.091 + (8.977 + mcdrag->rtr * (-12.804 + 8.38 * mcdrag->rtr)) * mcdrag->nsl) * s1 * b1 - a1;
	}
	else {
		b1 = sqrt(m2 - 1.0) / mach;
		b = 1.0528 + 0.23379 * b1 - 0.004884 * (mach - 1.0);
		a = 58.873 + (8.115 + (14.15 + mcdrag->rtr * (-15.348 + 7.216 * mcdrag->rtr)) * mcdrag->nsl) * s1 * b1 * b1 - a1;
	}
...
void mcdrag_calculate(lpMcDrag mcdrag)
{
	...
	if (mcdrag->alc)
		atmos_standardalt(mcdrag->atmos);
	...
	for (i = 0; i < DRAG_VELCOUNT; i++) {
		...
		data.cd_total = data.cd_nose + data.cd_bttl + data.cd_base + data.cd_rbnd + data.cd_skin;
